...
Release is manual and PR-first: provide the exact merged commit and its successful main CI run to [`.github/workflows/release.yml`](.github/workflows/release.yml). The workflow creates an annotated tag once, creates a draft release through the GitHub API, uploads the evidence assets, enables and verifies the platform immutable-release setting, publishes once, and verifies the annotated tag target, asset count, and GitHub asset digests. Failed runs, tags, drafts, releases, and pull requests are retained as evidence.
