...
func Generate(source, contractPath, fixturePath, historyPath, output, root string) (Report, error) {
	if !filepath.IsAbs(output) { return Report{}, fmt.Errorf("output must be an absolute caller-owned directory") }
	rootAbs, err := filepath.Abs(root)
	if err != nil { return Report{}, err }
	outputAbs, err := filepath.Abs(output)
	if err != nil { return Report{}, err }
	if isWithin(outputAbs, rootAbs) || outputAbs == rootAbs { return Report{}, fmt.Errorf("output must be outside the source repository") }
	if err := ensureEmptyOutput(outputAbs); err != nil { return Report{}, err }
	...
	if !replay.Deterministic { first.Report.Decision = DecisionRefuted }
	first.Human = renderHuman(first.Report, history)

	if err := writeProjection(outputAbs, first); err != nil { return Report{}, err }
	if err := VerifyConformance(first.Report); err != nil { return Report{}, err }
	return first.Report, nil
}

func Check(source, contractPath, fixturePath string) (SemanticGraph, Contract, Fixture, error) {
	graph, err := LoadGraph(source)
	if err != nil { return SemanticGraph{}, Contract{}, Fixture{}, err }
...
func buildProjection(graph SemanticGraph, contract Contract, fixture Fixture, history OperationalHistory, contractDigest, fixtureDigest string, inv InventoryReport) (Projection, error) {
	if len(graph.Cases) != len(contract.Cases) || len(contract.Cases) != len(fixture.Cases) { return Projection{}, fmt.Errorf("graph, contract, and fixture denominators do not have the same size") }
	if !semanticCasesMatch(graph.Cases, contract.Cases) { return Projection{}, fmt.Errorf(".gooo semantic cases and denominator contract disagree") }
	...
	for index, input := range fixture.Cases {
		if input.ID != contract.Cases[index].ID { return Projection{}, fmt.Errorf("fixture order mismatch at case %d", index+1) }
	...
	if stateCounts == (StateCounts{Closed: 4, Unknown: 4, Refuted: 4}) && expectedStatesMatch(cases, contract.Cases) { decision = DecisionClosed }
	...
	if utility.Unknown.NextOperation == "" { utility.Unknown.NextOperation = "collect-independent-user-workload-evidence" }
	...
	for key, data := range generated { artifactDigests[filepath.ToSlash(filepath.Join("generated", key))] = DigestBytes(data) }
	for _, receipt := range receipts { artifactDigests[filepath.ToSlash(filepath.Join("receipts", receipt.CaseID+".json"))] = DigestBytes(mustCanonical(receipt)) }
	for key, data := range layerReceipts { artifactDigests[filepath.ToSlash(filepath.Join("layer-receipts", key))] = DigestBytes(data) }
...
func authorityReport(fixture Fixture) AuthorityReport {
	...
	for _, item := range fixture.Cases {
		switch item.Layer {
		case "OPERATOR_AUTHORING": report.Operator.AuthoringEvents++
		case "ORCHESTRATOR_LOCAL":
			if item.EventKind == "LOCAL_VALIDATION" || item.CommandClass == "VALIDATION" { report.Orchestrator.LocalValidationEvents++ } else { report.Orchestrator.LocalAuthoringEvents++ }
		}
	}
	return report
}
...
func expectedStatesMatch(actual []CaseReport, expected []ContractCase) bool {
	...
	for index := range expected { if actual[index].ID != expected[index].ID || actual[index].Expected != expected[index].Expected { return false } }
	return true
}

func semanticCasesMatch(semantic []SemanticCase, contract []ContractCase) bool {
	if len(semantic) != len(contract) { return false }
	for index := range contract {
		if semantic[index].ID != contract[index].ID || semantic[index].Expected != contract[index].Expected || semantic[index].Kind != contract[index].Kind { return false }
	}
	return true
}
...
func contradictionReport(cases []CaseReport) ([]byte, error) {
	...
	for _, item := range cases { if item.Decision == DecisionRefuted { refuted = append(refuted, item) } }
	value := struct {
...
func receiptBytes(receipts []LayerReceipt) map[string][]byte {
	...
	for _, receipt := range receipts { result[receipt.CaseID+".json"] = mustCanonical(receipt) }
	return result
}

func buildLayerReceiptIndexes(receipts []LayerReceipt) (map[string][]byte, error) {
	byLayer := map[string][]LayerReceipt{}
	for _, receipt := range receipts { byLayer[receipt.Layer] = append(byLayer[receipt.Layer], receipt) }
	result := map[string][]byte{}
	layers := make([]string, 0, len(byLayer))
	for layer := range byLayer { layers = append(layers, layer) }
	sort.Strings(layers)
	for _, layer := range layers {
		...
		for _, item := range items { index.CaseIDs = append(index.CaseIDs, item.CaseID); index.EventIDs = append(index.EventIDs, item.EventID); index.ReceiptDigests = append(index.ReceiptDigests, item.ReceiptDigest) }
...
func equalByteMaps(left, right map[string][]byte) bool {
	...
	for key, value := range left { if !bytes.Equal(value, right[key]) { return false } }
	return true
}
...
func renderHuman(report Report, history OperationalHistory) []byte {
	...
	for index, command := range history.Commands { fmt.Fprintf(&builder, "%d. `%s`\n", index+1, command) }
...
func ensureEmptyOutput(path string) error {
	...
	if os.IsNotExist(err) { return os.MkdirAll(path, 0o755) }
	if err != nil { return err }
	if !info.IsDir() { return fmt.Errorf("output exists and is not a directory") }
	entries, err := os.ReadDir(path)
	if err != nil { return err }
	if len(entries) != 0 { return fmt.Errorf("output directory must be empty") }
	return nil
}

func writeProjection(output string, projection Projection) error {
	if err := os.MkdirAll(filepath.Join(output, "generated"), 0o755); err != nil { return err }
	if err := os.MkdirAll(filepath.Join(output, "receipts"), 0o755); err != nil { return err }
	if err := os.MkdirAll(filepath.Join(output, "layer-receipts"), 0o755); err != nil { return err }
	eventsPath := filepath.Join(output, "events.ndjson")
	file, err := os.OpenFile(eventsPath, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
	if err != nil { return err }
	if _, err := file.Write(projection.EventNDJSON); err != nil { file.Close(); return err }
	if err := file.Close(); err != nil { return err }
	for _, key := range sortedKeys(projection.Receipts) { if err := os.WriteFile(filepath.Join(output, "receipts", key), projection.Receipts[key], 0o644); err != nil { return err } }
	for _, key := range sortedKeys(projection.LayerReceipts) { if err := os.WriteFile(filepath.Join(output, "layer-receipts", key), projection.LayerReceipts[key], 0o644); err != nil { return err } }
	for _, key := range sortedKeys(projection.Generated) { if err := os.WriteFile(filepath.Join(output, "generated", key), projection.Generated[key], 0o644); err != nil { return err } }
	if err := os.WriteFile(filepath.Join(output, "contradiction-report.json"), projection.Contradiction, 0o644); err != nil { return err }
	if err := os.WriteFile(filepath.Join(output, "human-report.md"), projection.Human, 0o644); err != nil { return err }
	reportData, err := canonicalJSON(projection.Report)
	if err != nil { return err }
	if err := os.WriteFile(filepath.Join(output, "report.json"), reportData, 0o644); err != nil { return err }
	artifacts := make([]Artifact, 0)
	paths := []string{"events.ndjson", "contradiction-report.json", "human-report.md", "report.json"}
	for _, key := range sortedKeys(projection.Receipts) { paths = append(paths, filepath.ToSlash(filepath.Join("receipts", key))) }
	for _, key := range sortedKeys(projection.LayerReceipts) { paths = append(paths, filepath.ToSlash(filepath.Join("layer-receipts", key))) }
	for _, key := range sortedKeys(projection.Generated) { paths = append(paths, filepath.ToSlash(filepath.Join("generated", key))) }
...
func VerifyConformance(report Report) error {
	if report.Schema != "gooo/operational-provenance-projector/report/v1" || report.Decision != DecisionClosed { return fmt.Errorf("report is not CLOSED") }
	if report.Metrics.Denominator != 12 || report.Summary != (StateCounts{Closed: 4, Unknown: 4, Refuted: 4}) { return fmt.Errorf("report does not preserve exact 12-case denominator") }
	if len(report.Cases) != 12 { return fmt.Errorf("report case count is not 12") }
	for _, item := range report.Cases {
		if item.Decision == DecisionUnknown {
			if item.Unknown == nil || item.Unknown.Stage == "" || item.Unknown.Step == "" || item.Unknown.Reason == "" || item.Unknown.UnknownClass == "" || item.Unknown.NextOperation == "" || len(item.Unknown.BlockedBy) == 0 { return fmt.Errorf("UNKNOWN case %s does not contain the six required fields", item.ID) }
		}
		if item.Decision == DecisionRefuted && len(item.Refutations) == 0 { return fmt.Errorf("REFUTED case %s has no contradiction record", item.ID) }
	}
	if report.ReceiptChain.Length != 12 || !report.ReceiptChain.AppendOnly || !report.ReceiptChain.Valid { return fmt.Errorf("receipt chain is invalid") }
	if !report.Replay.Requested || !report.Replay.Deterministic || !report.Replay.EventsMatch || !report.Replay.ReceiptsMatch || !report.Replay.ReportsMatch { return fmt.Errorf("deterministic replay is not CLOSED") }
	runtime := report.Authority.Runtime
	if runtime.SourceRepositoryWrites != 0 || runtime.Commit != 0 || runtime.Merge != 0 || runtime.Tag != 0 || runtime.Release != 0 || runtime.LocalTestExecutions != 0 || runtime.CrossProjectRequiredGates != 0 { return fmt.Errorf("runtime authority is not zero") }
	if report.Authority.RepositoryWrites != 0 || report.Authority.LocalTestExecutions != 0 || report.Authority.CrossProjectRequiredGates != 0 { return fmt.Errorf("flat runtime authority is not zero") }
	if report.Authority.Operator.OperatorAPIAttempts != nil || report.Authority.Operator.OperatorAPIAttemptsState != DecisionUnknown { return fmt.Errorf("operator API attempts must remain null plus UNKNOWN") }
	if report.OperatorAPIAttempts != nil || report.OperatorAPIAttemptsState != DecisionUnknown { return fmt.Errorf("top-level operator API attempts must remain null plus UNKNOWN") }
	if report.OperationalAudit.State != "OPERATIONAL_REFUTED" || report.OperationalAudit.ExactCount != 5 || len(report.OperationalAudit.Commands) != 5 || !report.OperationalAudit.PreservedExistingRefuted || report.OperationalAudit.ExecutedByCurrentRuntime { return fmt.Errorf("v0.49 operational history was not preserved") }
	if report.Utility.Status != DecisionUnknown || report.Utility.Unknown.Stage == "" || report.Utility.Unknown.Step == "" || report.Utility.Unknown.Reason == "" || report.Utility.Unknown.UnknownClass == "" || report.Utility.Unknown.NextOperation == "" || len(report.Utility.Unknown.BlockedBy) == 0 { return fmt.Errorf("utility must remain UNKNOWN with six fields") }
	return nil
}
